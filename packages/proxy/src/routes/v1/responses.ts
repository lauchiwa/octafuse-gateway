 ...
 * - **能力门禁**：只保留显式声明 `endpoints.openai.endpoints.responses` 的 provider 路由。
 *   不从 `base` 派生（`listConfiguredCapabilities` 在有 `base` 时会返回全部能力，
 *   会把 10/42 个仅配 base 的预设误判为可用）。全部不可用才 502，单条不可用只过滤 + 记日志。
...
responsesRoutes.post('/', async (c) => {
  ...
  // 能力门禁：逐条过滤，不抛异常。若在驱动内 throw，failover-dispatch 会当成 fetch 失败，
  // 对该 provider 的每把 key 重试一轮，最终只回一个笼统的 502。
  const incapable: string[] = [];
  routes = routes.filter((route) => {
    if (providerDeclaresResponsesEndpoint(route.providerEndpoints)) return true;
    incapable.push(route.providerName || route.providerId);
    console.warn('[Gateway Responses] provider does not declare responses endpoint, skipping', {
  ...
  if (routes.length === 0) {
    return c.json(
      {
        error:
          `No provider for this model serves the Responses API. ` +
          `Configure endpoints.openai.endpoints.responses for: ${incapable.join(', ')}`,
      },
      502
    );
  }
